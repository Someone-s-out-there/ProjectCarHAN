...
void getUserTime() {
  ...
  while (1) {
    ...
    // If the value of a byte is 255, EEPROMByteNumber is increased by one and
    // the loop starts over If the value of a byte is not 255, the loop stops
    // and the program is ready to write
...
void updateUserTime() {
  ...
  if (currentMillis - previousMillisTime >= intervalTime) {
    ...
    // EEPROM current byte value is increased by one and is written to the
    // EEPROM
    EEPROMByteValue++;
    EEPROM.update(EEPROMByteNumber, EEPROMByteValue);

    // If the value of the current byte is 255 the program moves to the next
    // byte starting from 0
    if (EEPROMByteValue == 255) {
      EEPROMByteNumber++;
      EEPROMByteValue = 0;
    }
