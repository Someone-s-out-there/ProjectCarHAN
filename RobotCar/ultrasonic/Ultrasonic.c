...
static void Example_callback(uint8_t ovf_Count) {
    distance = (TCNT2 + (ovf_Count * 256)) /
               29;  // this can also be a global static variable
...
static US_complete_callback *FN =
    &Example_callback;  // set the default callback funtion. which does nothing
                        // useful except giving an example and preventing a
                        // segmentation fault for trying to call a function with
                        // the address of NULL
...
void ultrasonic_init(US_complete_callback *callback) {
    ...
    TCCR2A =
        0x00;  // no output / normal mode //set timeer overflow at 512 micro
    // seconds and 2 microseconds per clock tick
    TCCR2B = 0x00 | 0x03;  // prescalar 32;
    TIMSK2 = 0x01;         // enable overflow interrupt
}

void ultrasonic_set_callback(US_complete_callback *callback) { FN = callback; }
