...
void ultrasonic_init(void) {
    ...
    TCCR2A = 0x00; // no output / normal mode //set timeer overflow at 512 micro seconds and 2 microseconds per clock tick
...
uint8_t ultrasonic_get_distance(void) {
    ...
    uint16_t pulse_width = TCNT2 + (overflowcount * 256);  // Lees de pulsbreedte in

    // Bereken de afstand in centimeters en return het.
//	return (pulse_width * SOUND_SPEED) / (2 * F_CPU / 1000000); // The fuck is deze berekening
