...
/**
 * @brief waits until the uart data register is empty then writes the chData to the register
 */
...
/**
 * @brief loops over a passed in string pointer for each byte it calls @ref uart_putc
 */
...
/**
 * @brief interrupt service routine. this gets a character from UDR0(uart data register) and stores it in uint8_t c and places it at the end of the buffer. then checks if the character was a '\n' or the end of the buffer has been reached and marks the buffer.linecomplete.
 */
ISR(USART_RX_vect) {
    if (rxbuffer_p == NULL) {
      PORTB|=(1<<PORTB5);
